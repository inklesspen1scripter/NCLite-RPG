...
public OnPluginStart()
{
	...
	LoadTranslations("NCLiteRPG_frags.phrases");
...
public APLRes AskPluginLoad2(Handle myself,bool late,char[] error,int err_max)
{
	...
	CreateNative("NCLiteRPG_OpenMenuExchangeFrag", 	Native_OpenExchangeFragMenu);
	...
	if(IsValidPlayer(client))
	{
		...
		if(PlayerValue[client])
		{
			if(StrEqual(sArgs,"stop")||StrEqual(sArgs,"cancel")||StrEqual(sArgs,"стоп")||StrEqual(sArgs,"отмена"))
			{
				...
				NCLiteRPG_ChatMessage(client,sArgs);
			}
			else{
				if(StringIsNumeric(sArgs))
				{
					...
					NCLiteRPG_SetCredits(client,NCLiteRPG_GetCredits(client)+val*cfg_iPriceFrag);
...
void MenuExchangeFrags(int client)
{
	...
	else
	{
		...
		NCLiteRPG_ChatMessage(client, Message);
	}
}
...
public int HandlerMenuExchange(Menu menu, MenuAction action, int client, int param2){
	if(action == MenuAction_Select)
	{
		...
		if(StrEqual(info,"all")){
			...
			NCLiteRPG_SetCredits(client,NCLiteRPG_GetCredits(client)+frags*cfg_iPriceFrag);
		...
		else
		{
			...
			NCLiteRPG_SetCredits(client,NCLiteRPG_GetCredits(client)+(frags-value));
