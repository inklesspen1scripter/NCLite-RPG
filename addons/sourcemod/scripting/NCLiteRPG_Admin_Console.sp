...
{
	RegConsoleCmd("NCLiteRPG_admin_reload", Command_Reload, 		"NCLiteRPG Admin Command");
	RegConsoleCmd("NCLiteRPG_xp",	         Command_NCLiteRPG_XP, 		"NCLiteRPG Admin Command");
	RegConsoleCmd("NCLiteRPG_credits", 	 Command_NCLiteRPG_CREDITS, 	"NCLiteRPG Admin Command");
	RegConsoleCmd("NCLiteRPG_level",  	 Command_NCLiteRPG_LEVEL,   	"NCLiteRPG Admin Command");
	RegConsoleCmd("NCLiteRPG_upgrade",	 Command_NCLiteRPG_UPGRADE, 	"NCLiteRPG Admin Command");
	RegConsoleCmd("NCLiteRPG_reset",  	 Command_NCLiteRPG_RESET,	"NCLiteRPG Admin Command");
	RegConsoleCmd("NCLiteRPG_remove", 	 Command_NCLiteRPG_REMOVE, 	"NCLiteRPG Admin Command");
	LoadTranslations("NCLiteRPG_admin_console.phrases");
	FirstLog=true;
}
...
bool LoadAdminConfig() {
	...
	if(cfg_bLogs)
	{
		RPG_Configs.GetString("logs","admins_path",cfg_sLogPath, sizeof cfg_sLogPath, "addons/sourcemod/logs/NCLiteRPG/admins");
...
public Action Command_Reload(int client,int args) {
	...
	if(!CheckAdminAccess(client)) {
		...
		NCLiteRPG_ChatMessage(client,buffer); return Plugin_Handled;
	}
	if(!LoadAdminConfig()){
...
public Action Command_NCLiteRPG_XP(int client,int args) {
	char buffer[PLATFORM_MAX_PATH];
	if(!CheckAdminAccess(client)) {
		FormatST("No Access",buffer,sizeof buffer,client,"No Access");
		NCLiteRPG_ChatMessage(client,buffer); return Plugin_Handled;
	...
	if(mode<0) { PrintToConsole(client,"Use: NCLiteRPG_xp <take/set/add> <steamid> <amount>" ); return Plugin_Handled; }
	...
	if(target==-1){ if(NCLiteRPG_GetParamStringBySteamID(buffer,"name",name,sizeof name)) target = 0; }
	...
	if(mode==0)
	{
		...
		if(target>0)
		{
			if(client!= target){
				...
				NCLiteRPG_ChatMessage(target,msg); // Сообщение для цели
			}
			ShowMessageToAdmin(client,"%s took %d XP a player %s","Admin: take xp",AdminName, amount,name); //Сообщение админам
			NCLiteRPG_TakeExp(target, amount, true, false);
		}
		else if(target==0) NCLiteRPG_TakeExpBySteamID(buffer, amount);
	}
	else if(mode==1)
	{
		GetAdminName(client, AdminName, sizeof(AdminName)); 
		...
		if(target>0)
		{
			...
			NCLiteRPG_SetExp(target, amount, false, true, false, false, false);
		}
		else if(target==0) NCLiteRPG_SetExpBySteamID(buffer,amount, false, true,false);
	}
	else if(mode==2)
	{
		GetAdminName(client, AdminName, sizeof(AdminName));
		...
		if(target>0)
		{
			...
			NCLiteRPG_GiveExp(target, amount, true, false, false, false,EVENT_NONE);
		}
		else if(target==0)  NCLiteRPG_GiveExpBySteamID(buffer,amount,true, false);
	}
	return Plugin_Handled;
}

public Action Command_NCLiteRPG_CREDITS(int client,int args) {
	char buffer[PLATFORM_MAX_PATH];
	if(!CheckAdminAccess(client)) {
		FormatST("No Access",buffer,sizeof buffer,client,"No Access");
		NCLiteRPG_ChatMessage(client,buffer); return Plugin_Handled;
	...
	GetCmdArg(1, buffer, sizeof(buffer));
	int mode = GetModeAction(buffer);
	if(mode<0) { PrintToConsole(client,"Use: NCLiteRPG_credits <take/set/add> <steamid> <amount>" ); return Plugin_Handled; }
	GetCmdArg(3, buffer, sizeof(buffer)); int amount = StringToInt(buffer); GetCmdArg(2, buffer, sizeof(buffer));
	int target = IsConnectedPlayer(buffer); char name[32];char AdminName[32];
	if(target==-1){ if(NCLiteRPG_GetParamStringBySteamID(buffer,"name",name,sizeof name)) target = 0; }
	else GetClientName(target, name, sizeof name);
	...
	if(mode==0)
	{
		...
		if(target>0)
		{
			...
			NCLiteRPG_SetCredits(target, NCLiteRPG_GetCredits(target)-amount);
		}
		else if(target==0) NCLiteRPG_SetParamIntBySteamID(buffer,"credits", NCLiteRPG_GetParamIntBySteamID(buffer,"credits")-amount);
	}
	else if(mode==1)
	{
		GetAdminName(client, AdminName, sizeof(AdminName)); 
		...
		if(target>0)
		{
			...
			NCLiteRPG_SetCredits(target, amount);
		}
		else if(target==0) NCLiteRPG_SetParamIntBySteamID(buffer,"credits", amount);
	}
	else if(mode==2)
	{
		GetAdminName(client, AdminName, sizeof(AdminName));
		...
		if(target>0)
		{
			...
			NCLiteRPG_GiveCredits(target,amount,EVENT_NONE);
		}
		else if(target==0) NCLiteRPG_SetParamIntBySteamID(buffer,"credits", NCLiteRPG_GetParamIntBySteamID(buffer,"credits")+amount);
	}
	return Plugin_Handled;
...
public Action Command_NCLiteRPG_LEVEL(int client,int args) {
	char buffer[PLATFORM_MAX_PATH];
	if(!CheckAdminAccess(client)) {
		FormatST("No Access",buffer,sizeof buffer,client,"No Access");
		NCLiteRPG_ChatMessage(client,buffer); return Plugin_Handled;
	...
	GetCmdArg(1, buffer, sizeof(buffer));
	int mode = GetModeAction(buffer);
	if(mode<0) { PrintToConsole(client,"Use: NCLiteRPG_level <take/set/add> <steamid> <amount>" ); return Plugin_Handled; }
	GetCmdArg(3, buffer, sizeof(buffer)); int amount = StringToInt(buffer); GetCmdArg(2, buffer, sizeof(buffer));
	int target = IsConnectedPlayer(buffer); char name[32];char AdminName[32];
	if(target==-1){ if(NCLiteRPG_GetParamStringBySteamID(buffer,"name",name,sizeof name)) target = 0; }
	else GetClientName(target, name, sizeof name);
	...
	if(mode==0)
	{
		...
		if(target>0)
		{
			...
			NCLiteRPG_SetLevel(target, NCLiteRPG_GetLevel(target)-amount);
		}
		else if(target==0) NCLiteRPG_SetParamIntBySteamID(buffer,"level", NCLiteRPG_GetParamIntBySteamID(buffer,"level")-amount);
	}
	else if(mode==1)
	{
		GetAdminName(client, AdminName, sizeof(AdminName)); 
		...
		if(target>0)
		{
			...
			NCLiteRPG_SetLevel(target,amount);
		}
		else if(target==0) NCLiteRPG_SetParamIntBySteamID(buffer,"level", amount);
	}
	else if(mode==2)
	{
		GetAdminName(client, AdminName, sizeof(AdminName));
		...
		if(target>0)
		{
			...
			NCLiteRPG_SetLevel(target,NCLiteRPG_GetLevel(target)+amount);
			NCLiteRPG_GiveCredits(target,amount*cfg_iCreditsInc,EVENT_NONE);
		}
		else if(target==0){
			NCLiteRPG_SetParamIntBySteamID(buffer,"level", NCLiteRPG_GetParamIntBySteamID(buffer,"level")+amount);
			NCLiteRPG_SetParamIntBySteamID(buffer,"credits", cfg_iCreditsInc*amount);
		}
	}
	return Plugin_Handled;
}
...
public Action Command_NCLiteRPG_UPGRADE(int client,int args) {
	char buffer[PLATFORM_MAX_PATH];
	if(!CheckAdminAccess(client)) {
		FormatST("No Access",buffer,sizeof buffer,client,"No Access");
		NCLiteRPG_ChatMessage(client,buffer); return Plugin_Handled;
	...
	GetCmdArg(1, buffer, sizeof(buffer));
	int mode = GetModeAction(buffer);
	if(mode<0) { PrintToConsole(client,"Use: NCLiteRPG_upgrade <take/set/add> <steamid> <shortname> <amount>" ); return Plugin_Handled; }
	GetCmdArg(3, buffer, sizeof(buffer)); int amount = StringToInt(buffer); GetCmdArg(2, buffer, sizeof(buffer));
	...
	int skillid = NCLiteRPG_FindSkillByShortname(shortname);
	if(skillid==-1) { PrintToConsole(client,"shortname is invalid" ); return Plugin_Handled; }
	int skillmax =NCLiteRPG_GetSkillMaxLevel(skillid);
	if(mode==0)
	{
		GetAdminName(client, AdminName, sizeof(AdminName)); 
		if(cfg_bLogs){
		...
		int skilllvl =NCLiteRPG_GetSkillLevel(target,skillid)-amount;
		...
		if(target>0)
		{
			...
			NCLiteRPG_SetSkillLevel(target, skillid,skilllvl);
		}
		else if(target==0){
			Format(shortname,sizeof shortname,"_%s",shortname);
			NCLiteRPG_SetParamIntBySteamID(buffer,shortname, skilllvl);
		}
	}
	else if(mode==1)
	{
		...
		if(target>0)
		{
			...
			NCLiteRPG_SetSkillLevel(target, skillid,amount);
		}
		else if(target==0){
			Format(shortname,sizeof shortname,"_%s",shortname);
			NCLiteRPG_SetParamIntBySteamID(buffer,shortname, amount);
		}
	}
	else if(mode==2)
	{
...
public Action Command_NCLiteRPG_RESET(int client,int args) {
	char buffer[PLATFORM_MAX_PATH];
	if(!CheckAdminAccess(client)) {
		FormatST("No Access",buffer,sizeof buffer,client,"No Access");
		NCLiteRPG_ChatMessage(client,buffer); return Plugin_Handled;
	...
	if(target>0)
	{
		...
		NCLiteRPG_ResetPlayer(target);
	}
	else if(target==0){
		PrintToConsole(client,"Use: NCLiteRPG_remove <steamid>" );
	}
	return Plugin_Handled;
}

public Action Command_NCLiteRPG_REMOVE(int client,int args) {
	char buffer[PLATFORM_MAX_PATH];
	if(!CheckAdminAccess(client)) {
		FormatST("No Access",buffer,sizeof buffer,client,"No Access");
		NCLiteRPG_ChatMessage(client,buffer); return Plugin_Handled;
	...
	if(target>0)
	{
		...
		if(IsValidPlayer) NCLiteRPG_ChatMessage(client,msg); // Сообщение для админа
	...
	if(cfg_bShowAdminsMessage)
	{
		for(int client = 1; client <= MaxClients; client++)
		{
			if(IsValidPlayer(client) && CheckAdminAccess(client) && client!=i)
			{
				...
				NCLiteRPG_ChatMessage(client,msg);
			}
		}
	}
	else if(i!=0)
