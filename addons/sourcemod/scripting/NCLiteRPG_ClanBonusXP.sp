...
{
	NCLiteRPG_Configs RPG_Configs = NCLiteRPG_Configs(ThisAddonName,CONFIG_ADDON);
	...
	LoadTranslations ("NCLiteRPG_clanbonus_xp.phrases.txt");
	LoadTranslations ("NCLiteRPG.phrases.txt");
}

public Action NCLiteRPG_OnPlayerGiveExpPre(int client, &Exp)
{
...
public Action WelcomeAdvertTimer (Handle timer, int client)
{
	...
	if (strlen(cfg_ClanName)>0 && IsValidPlayer(client)) 
	{
		...
		NCLiteRPG_ChatMessage (client, "%T", "Welcome",client,ClientName,buffer2);
		if (xprate==0) NCLiteRPG_ChatMessage (client, "%T", "Welcome_No_Bonus",client);
		else NCLiteRPG_ChatMessage(client, "%T", "Welcome_XP",client,str_xprate);
	}
	return Plugin_Stop;
}