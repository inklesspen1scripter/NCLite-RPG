...
#define AbilityUp "NCLiteRPG/ability_up1.mp3"

public void OnMapStart()
{
	KeyValues hConfig = new KeyValues("NCLiteRPG");
	char full_path[512];
	FormatEx(full_path,sizeof full_path,"configs/NCLiteRPG/Addon.%s.txt",ThisAddonName);
	...
	if(EffectCount>0)
	{
		AddFolderDecalToDownloadsTable("materials/overlays/NCLiteRPG");
		AddFolderSoundToDownloadsTable("NCLiteRPG");
		PrecacheSound(AbilityUp);
	}
}

public void NCLiteRPG_OnPlayerLevelUp(int client,int level)
{
	if(EffectCount>0)
	{
