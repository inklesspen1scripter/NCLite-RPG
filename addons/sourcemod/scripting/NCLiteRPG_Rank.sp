...
{
	char buffer[256];
	FormatEx(buffer,sizeof buffer,"SELECT COUNT(*) FROM nc_rpg WHERE level > %d OR (level = %d AND xp > %d) OR (level = %d AND xp = %d AND credits > %d)",NCLiteRPG_GetLevel(client),NCLiteRPG_GetLevel(client),NCLiteRPG_GetXP(client),NCLiteRPG_GetLevel(client),NCLiteRPG_GetXP(client),NCLiteRPG_GetCredits(client));
...
{
	if (results == null) { NCLiteRPG_LogMessage(LogType_Error, "Could not get rank Player %N, reason: %s",client, error); return; }
	if(results.HasResults)
	{
		int rank = results.FetchInt(0)+1; char msg[256];
		FormatST("Your position in the top %i! You are at level %i, %i XP and %i credits",msg,sizeof msg,client,"Your rank", rank, NCLiteRPG_GetLevel(client), NCLiteRPG_GetXP(client), NCLiteRPG_GetCredits(client));
		NCLiteRPG_ChatMessage(client,msg);
	}
}
...
{
	if (results == null) { NCLiteRPG_LogMessage(LogType_Error, "Could not get Top Players, reason: %s", error); return; }
