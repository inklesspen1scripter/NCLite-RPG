...
public Plugin myinfo = {
	name		= "NCLiteRPG Tesla Coil",
	author		= "SenatoR",
	description	= "Skill Tesla Coil for NCLiteRPG",
	version		= VERSION,
	url			= ""
};
...
public void NCLiteRPG_OnPlayerSpawn(int client) {
	...
	if(NCLiteRPG_GetSkillLevel(client, ThisSkillID) > 0) hTimerTeslaCoil[client] = CreateTimer(cfg_fInterval, Timer_TeslaCoil, client, TIMER_REPEAT|TIMER_FLAG_NO_MAPCHANGE);
}
...
void TeslaCoil(int client,int level)
{
	...
	for(int attacker = 1; attacker <= MaxClients; ++attacker)
	{
		...
		if(GetVectorDistance(AttackerPos, ClientPos, false) < Range)
		{
			...
			{
				if(NCLiteRPG_SkillActivate(ThisSkillID,client,attacker)>= Plugin_Handled) return;
				if(cfg_bEffects)
				{
				...
				if(amount)
				{
					NCLiteRPG_DealDamage(attacker, amount, client, DMG_SHOCK, "weapon_taser");
