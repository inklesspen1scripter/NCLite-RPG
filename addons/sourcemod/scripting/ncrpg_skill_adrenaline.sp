...
public void OnPluginStart() { if((ThisSkillID = NCLiteRPG_FindSkillByShortname(ThisSkillShortName)) == -1)
	{
	...
	if(IsValidPlayer(victim) && IsValidPlayer(attacker) && g_hPlayerIsAdrenalined[victim]==INVALID_HANDLE)
	{
		...
		if(level > 0)
		{
			if(GetRandomFloat(0.0, 1.0) <= cfg_fChance*level)
			{
				...
				NCLiteRPG_Buffs(victim).Speed = NCLiteRPG_Buffs(victim).Speed+(level*cfg_fAmount);
...
{
	NCLiteRPG_Buffs(client).Speed = NCLiteRPG_Buffs(client).MaxSpeed;
...
{
	if(IsValidPlayer(client)) NCLiteRPG_SetSpeed(client,NCLiteRPG_GetMaxSpeed(client));
