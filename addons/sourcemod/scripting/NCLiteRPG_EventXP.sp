...
public Action Event_PlayerDeath(Event event, const char[] name, bool dontBroadcast) {
	...
	if(IsValidPlayer(assister) && IsValidPlayer(victim))
	{
		...
		if(cfg_iExpAssist[0]) xp=NCLiteRPG_GetLevel(victim)*xp/NCLiteRPG_GetLevel(attacker);
		if(xp>0) NCLiteRPG_GiveExp(attacker, xp, true);
	}
	if(IsValidPlayer(attacker) && IsValidPlayer(victim) && victim != attacker)
	{
		int xp = GetRandomInt(cfg_iExpKill[1],cfg_iExpKill[2]);
		if(cfg_iExpKill[0]) xp=NCLiteRPG_GetLevel(victim)*xp/NCLiteRPG_GetLevel(attacker);
		if(xp>0)
		{
			char buffer[32];
			...
			{	
				if(cfg_iExpKnife[0]) xp+= NCLiteRPG_GetLevel(victim)*GetRandomInt(cfg_iExpKnife[1],cfg_iExpKnife[2])/NCLiteRPG_GetLevel(attacker);
				else xp+=GetRandomInt(cfg_iExpKnife[1],cfg_iExpKnife[2]);
			}
			if(event.GetBool("headshot"))
			{
				if(cfg_iExpHeadshot[0]) xp+= NCLiteRPG_GetLevel(victim)*GetRandomInt(cfg_iExpHeadshot[1],cfg_iExpHeadshot[2])/NCLiteRPG_GetLevel(attacker);
				else xp+=GetRandomInt(cfg_iExpHeadshot[1],cfg_iExpHeadshot[2]);
			}
			NCLiteRPG_GiveExp(attacker, xp, true);
		}
		else NCLiteRPG_LogMessage(LogType_Error,"Player %N got negative Xp Info: XP = %d, Victim Level = %d, Attacker Level = %d [exp_kill]",attacker,xp,NCLiteRPG_GetLevel(victim),NCLiteRPG_GetLevel(attacker));
	}
...
public Event_RoundEnd(Event event, const char[] name, bool dontBroadcast) {
	...
	if(winner>1)
		for(int client = 1; client <= MaxClients; client++)
			if(IsValidPlayer(client) && GetClientTeam(client) == winner)  NCLiteRPG_GiveExp(client, GetExpResult(winner,cfg_iExpWin));
}

public Event_BombPlanted(Event event, const char[] name, bool dontBroadcast) {
	int client = GetClientOfUserId(event.GetInt("userid"));
	if(IsValidPlayer(client)) NCLiteRPG_GiveExp(client, GetExpResult(client,cfg_iExpPlanted));
}

public Event_BombExploded(Event event, const char[] name, bool dontBroadcast) {
	int client = GetClientOfUserId(event.GetInt("userid"));
	if(IsValidPlayer(client)) NCLiteRPG_GiveExp(client, GetExpResult(client,cfg_iExpExploded));
}

public Event_BombDefused(Event event, const char[] name, bool dontBroadcast) {
	int client = GetClientOfUserId(event.GetInt("userid"));
	if(IsValidPlayer(client)) NCLiteRPG_GiveExp(client, GetExpResult(client,cfg_iExpDefused));
}

public Action Event_RoundMVP(Event event, const char[] name, bool dontBroadcast)
{
	int client = GetClientOfUserId(event.GetInt("userid"));
	if(IsValidPlayer(client)) NCLiteRPG_GiveExp(client, GetExpResult(client,cfg_iExpMVP));
}
